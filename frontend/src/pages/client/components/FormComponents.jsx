...
export const InputField = ({ label, icon, ...props }) => (
  <div className="space-y-2">
    <label className="flex items-center space-x-2 text-sm sm:text-base font-medium text-gray-700">
      {icon}
      <span>{label}</span>
    </label>
    <input
...
export const SelectField = ({ label, icon, children, ...props }) => (
  <div className="space-y-2">
    ...
    <select
      {...props}
      className="w-full px-3 py-2 text-sm sm:text-base border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 bg-white text-gray-900"
    >
      {children}
    </select>
  </div>
