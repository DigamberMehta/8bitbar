...
const CheckoutPage = () => {
  ...
  const renderInputField = (name, label, placeholder, required = false, type = 'text') => (
    <div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-16">
          ...
          <div className="lg:w-2/3 bg-black/60 border border-pink-500/30 rounded-lg p-8 shadow-lg">
            <h2 className="font-['Orbitron'] text-2xl font-bold text-white mb-6">Billing Details</h2>
            <form className="space-y-6">
              ...
              {renderInputField('street', 'Street Address', 'House number and street name', true)}
              {renderInputField('apartment', 'Apartment, suite, etc.', 'Optional')}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {renderInputField('suburb', 'Suburb', 'e.g., Fortitude Valley', true)}
                {renderInputField('postcode', 'Postcode', 'e.g., 4006', true)}
              </div>
              {renderInputField('phone', 'Phone', 'Optional', false, 'tel')}
              {renderInputField('email', 'Email Address', 'you@wonderland.com', true, 'email')}
            </form>
          </div>
          ...
          <div className="lg:w-1/3">
            <div className="bg-black/60 border border-pink-500/30 rounded-lg p-8 shadow-lg sticky top-24">
              ...
              <div className="space-y-4 mb-6">
                {cart.map((item, index) => (
                  <div key={index} className="flex justify-between text-gray-300">
                    <span>{item.title || item.roomName} x {item.duration}h</span>
              ...
              <div className="mt-8">
                 <h3 className="font-['Orbitron'] text-xl font-bold text-white mb-4">Payment Details</h3>
              ...
              <button
                ...
                className="w-full mt-8 py-3 bg-gradient-to-r from-cyan-400 to-pink-500 text-white font-bold rounded-lg shadow-lg hover:scale-105 transition-all duration-300 text-lg"
              >
                Place Order
              </button>
            </div>
